...
_SHARED_SECRET = (
    os.environ.get("API_SHARED_SECRET")
    or os.environ.get("GUSTOPEDIA_API_SHARED_SECRET")
    or os.environ.get("GNAMMY_API_SHARED_SECRET")
    or ""
)
...
def require_auth(credentials: Optional[HTTPAuthorizationCredentials] = Depends(security)) -> None:
    if not _SHARED_SECRET:
        logger.warning("require_auth: API_SHARED_SECRET/GUSTOPEDIA_API_SHARED_SECRET is not configured or empty.")
        return
